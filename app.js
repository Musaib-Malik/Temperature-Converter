...
function convert(e) {
  ...
  const celsiusToFahrenheit = parseFloat(
    (tempInput.value * 9) / 5 + 32
  ).toFixed(2);
  const celsiusToKelvin = parseFloat(tempInput.value) + 273.15;

  // Fahrenheit to Celsius & Kelvin
  const fahrenheitToCelsius = parseFloat(
    ((tempInput.value - 32) * 5) / 9
  ).toFixed(2);
  const fahrenheitToKelvin =
    (parseFloat(tempInput.value - 32) * 5) / 9 + 273.15;

  // Kelvin to Celsius & Fahrenheit
  const kelvinToCelsius = parseFloat(tempInput.value - 273.15).toFixed(2);
  const kelvinToFahrenheit = parseFloat(
    ((tempInput.value - 273.15) * 9) / 5 + 32
  ).toFixed(2);
